...
# Ensure MySQL database healthcare is manually created first and set as the default database

### Bronze Level Tables

# create table: bronze_admission_type
CREATE TABLE IF NOT EXISTS bronze_admission_type (
...
# create table discharge_disposition: bronze_discharge_disposition
CREATE TABLE IF NOT EXISTS bronze_discharge_disposition (
...
# create table admission_source: bronze_admission_source
CREATE TABLE IF NOT EXISTS bronze_admission_source (
...
# create table patient: bronze_patient
...
# create patient encounter table: bronze_patient_encounter
CREATE TABLE IF NOT EXISTS bronze_patient_encounter (
  encounter_id BIGINT COMMENT 'Unique identifier for each patient encounter or hospital visit.',
	...
	payer_code VARCHAR(5) COMMENT 'Type of payment source (e.g., MC = Medicare, MD = Medicaid, SP = Self-Pay).',
  ...
  glyburideMetformin VARCHAR(10) COMMENT 'Whether the drug metformin was prescribed (e.g., No drug not prescribed, Steady, Up, Down).',
	...
	change_in_meds	VARCHAR(5) COMMENT 'Whether there was a change in diabetic medications during the encounter.  Values: change and no change',
